(function () {
    ...
    var detailsWatchLaterPill = document.getElementById('video-watchlater-pill');
    ...
    var currentChannelId = (document.body?.dataset?.currentChannelId) || null;
    var currentVideoElement = null;
    var currentVideoId = null;
    var hideVideoButton = document.getElementById('btn-hide-video');
    var watchLaterButton = document.getElementById('btn-watch-later');
    // Referencias de modal de canal (aunque la lógica esté comentada, evitar ReferenceError)
    var channelModalOverlay = document.getElementById('channel-modal-overlay') || null;
    var channelModalClose = document.getElementById('channel-modal-close') || null;
    var channelModalTitle = document.getElementById('channel-modal-title') || null;
    var channelModalMeta = document.getElementById('channel-modal-meta') || null;
    var channelModalStatus = document.getElementById('channel-modal-status') || null;
    var channelModalOpenBtn = document.getElementById('channel-modal-open') || null;
    var channelModalScanBtn = document.getElementById('channel-modal-scan') || null;
    ...
    function setupLazyChatPhotos(root) {
        try {
            ...
            var imgs = scope.querySelectorAll('.lazy-chat-photo[data-src]');
            imgs.forEach(function (img) { observer.observe(img); });

            // Exponer para reutilizar tras renders dinámicos
            window.refreshLazyChatPhotos = function (r) {
                var sc = r || document;
                var imgs2 = sc.querySelectorAll('.lazy-chat-photo[data-src]');
    ...
    function applyWatchLaterStateToElement(el) {
        if (!el) return;
        var watchLaterFlag = el.dataset.watchLater === '1';
        if (watchLaterFlag) {
            el.classList.add('is-watchlater');
        } else {
            el.classList.remove('is-watchlater');
        }
        var indicator = el.querySelector('.video-watchlater-indicator');
        if (indicator) {
    ...
    function updateWatchLaterButton() {
        ...
        var isSaved = currentVideoElement.dataset.watchLater === '1';
        var icon = watchLaterButton.querySelector('i');
        var span = watchLaterButton.querySelector('span');
        if (icon) {
    ...
    function fillVideoDetailsFromElement(el) {
        ...
        if (detailsWatchLaterPill) {
            detailsWatchLaterPill.style.display = watchLaterFlag ? 'block' : 'none';
        }
    }
    ...
    if (searchForm) {
        ...
        let limitContainer = document.getElementById('limites');
        ...
        async function performSearch(ev) {
            ...
            console.log('[search] submit', { q, scope, type, limit, currentChannelId });
            if (!q) return;
            ...
            if (paginationBar) paginationBar.remove(); // eliminar paginación del listado general durante la búsqueda

            try {
                const params = new URLSearchParams({ q, scope, type, limit: String(limit) });
                if (currentChannelId) params.set('chat_id', currentChannelId);

                // Actualizar la URL (GET) con los parámetros de búsqueda
                ...
                // Gestionar título y control de límite
                ...
                const resultsText = `Resultados de la busqueda (${formatThousands(items.length)})`;

                // Asegurar que el control de límite exista y sea visible
                if (!limitContainerDyn && titleBar) {
                    ...
                    // Reasignar referencias al input recién creado
        ...
        // Si la URL ya trae parámetros de búsqueda, rellena y ejecuta
        ...
        if (initialQ) {
            ...
            if (initialChat) currentChannelId = initialChat;
            // Ejecuta búsqueda inicial sin requerir submit manual
            performSearch();
        }
    }
    ...
    // Delegamos el click para soportar contenido generado dinámicamente (videos)
    document.addEventListener('click', function (event) {
        var target = event.target.closest('.file-item[data-item-type="video"]');
        if (!target) return;
        var streamUrl = target.dataset.streamUrl || target.dataset.streamUrl;
        if (!streamUrl) return;
    ...
    document.addEventListener('click', function (event) {
        var btn = event.target.closest('.open-channel-modal');
        if (!btn) return;
        event.preventDefault();
    ...
    if (channelModalOverlay) {
        channelModalOverlay.addEventListener('click', function (ev) {
            if (ev.target === channelModalOverlay) closeChannelModal();
        });
    }
    // Botón abrir canal
    if (channelModalOpenBtn) {
        channelModalOpenBtn.addEventListener('click', openChannelLink);
    }
    // Botón indexar faltantes
    if (channelModalScanBtn) {
        channelModalScanBtn.addEventListener('click', triggerChannelScan);
    }
    ...
    (function setupFolderWS() {
        try {
            ...
            // Reuso la función existente de renderizado si está disponible, de lo contrario fallback simple
            function renderFolderItems(items) {
                ...
                // Fallback minimal: solo muestra conteo
    ...
    function setupLiveFolderUpdates() {
        ...
        function renderItems(items) {
            ...
            items.forEach(function (item) {
                ...
                var photoUrl = item.photo_url || (photoId ? ('/api/photo/' + encodeURIComponent(photoId) + '?tipo=grupo') : null);
