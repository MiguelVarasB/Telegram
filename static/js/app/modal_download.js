(function () {
    ...
    function renderDownloads(list) {
        ...
        const rows = list.map(item => {
            ...
            const canCancel = status === 'downloading' && item.chat_id && item.message_id && item.video_id;
            ...
            return `
                <div class="download-row download-status-${status}">
                    ...
                    ${canCancel ? `<button class="download-cancel-btn" data-cancel="${item.chat_id}:${item.message_id}:${item.video_id}">Cancelar</button>` : ''}
                </div>
            `;
        }).join('');
        ...
        // Wire cancel buttons
        dom.downloadsContent.querySelectorAll('.download-cancel-btn').forEach(btn => {
            btn.addEventListener('click', async (ev) => {
                ev.stopPropagation();
                const triple = btn.dataset.cancel;
                if (!triple) return;
                const [chatId, messageId, videoId] = triple.split(':');
                btn.disabled = true;
                try {
                    await fetch(`/api/downloads/cancel/${chatId}/${messageId}/${videoId}`, { method: 'POST' });
                } catch (e) {
                    console.warn('No se pudo cancelar', e);
                } finally {
                    btn.disabled = false;
                }
            });
